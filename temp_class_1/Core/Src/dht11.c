...
#define htim htim2 // You must set your clock tree and TIM1 (or other timer) to generate an interruption for each us (1MHz frequency)
  ...
  if (DHT11_setup()) {
    ...
    for (j = 0; j < 5; j++) {
      for (i = 0; i < 8; i++) {
        ...
        delay_us(30);  // FIXED: Wait 30us (not 40us) to sample bit

        if (HAL_GPIO_ReadPin(Dstruct.port, Dstruct.pin)) {
          value |= (1 << (7 - i));  // Bit 1 (70us high)
        }
    ...
    Dstruct.tempInt = values[2];    // INTEGER part (0-50°C)
    Dstruct.tempDec = values[3];    // DECIMAL part (0)
